...
const Faq = () => {
  ...
  //Herewe used halfLength to divide whole faq in two parts
  return (
    <section>
      <Element name="faq" className="relative">
        ...
        <div className='faq-glow_before relative z-2 border-2 border-s2 bg-s1'>
          <div className='container flex gap-10 max-lg:block'>
            ...
            {/* slice, creates a new array with the part of array, and 
            map also create a new array by performing function on each array */}
              {faq.slice(0, halfLength).map((item, index) => (
                <FaqItem key={item.id} item={item} index={index} />
              ))}
            </div>
            <div className='relative flex-1 lg:pt-24'>
            {/* Faq is new component we used to display questions and answers */}
